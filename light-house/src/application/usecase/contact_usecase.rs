    ...
    T: ContactRepositoryBase + ContactRepositoryUtility + Send + Sync
{
    ...
    {
        // Step 1: Create the contact in the database
        let contact_created = match self.contact_repository.create(user_id, contact_dto).await {
            Ok(contact) => contact,
            Err(err) => return Err(UsecaseError::from(err)),
        };

        // Step 2: Fetch the contact type name using the contact_type_id
        let contact_type_name = match self
            .contact_repository
            .find_by_id(match Uuid::from_slice(&contact_created.contact_type_id) {
                Ok(uuid) => uuid,
        ...
        let res_entry = ResEntryContactDto {
            id: match String::from_utf8(contact_created.id) {
                Ok(id) => id,
                Err(err) => return Err(UsecaseError::Unexpected(err.to_string())),
            },
        ...
        -> Result<Option<ResEntryContactDto>, UsecaseError>
    {
        ...
        let contact = match self.contact_repository.find_by_user_id_and_contact_id(user_id, contact_id).await {
            Ok(Some(contact)) => {
                ...
                let contact_type_name = match self
                    .contact_repository
                    .find_by_id(match Uuid::from_slice(&contact.contact_type_id) {
                        Ok(uuid) => uuid,
                        Err(err) => return Err(UsecaseError::Unexpected(err.to_string())),
                    })
                    .await
                ...
                Some(ResEntryContactDto {
                    id: match String::from_utf8(contact.id) {
                        Ok(id) => id,
                        Err(err) => return Err(UsecaseError::Unexpected(err.to_string())),
                    },
                    name: contact.name,
        ...
        -> Result<ResEntryContactDto, UsecaseError>
    {
        ...
        match result {
            Ok(updated_contact) => {
                ...
                let contact_type_name = match self
                    .contact_repository
                    .find_by_id(match Uuid::from_slice(&updated_contact.contact_type_id) {
                        Ok(uuid) => uuid,
                        Err(err) => return Err(UsecaseError::Unexpected(err.to_string())),
                    })
                    .await
                ...
                let res_entry = ResEntryContactDto {
                    id: match String::from_utf8(updated_contact.id) {
                        Ok(id) => id,
