...
pub trait ContactTypeRepositoryUtility: Send + Sync {
    async fn find_by_name(&self, name: &str, user_id: Uuid) -> Result<Option<contact_type::Model>, RepositoryError>;
    async fn find_all_by_user_id(&self, user_id: Uuid) -> Result<Vec<contact_type::Model>, RepositoryError>;
}
